...
function mockServe({directory = './', responseDelay = 0, cacheLifetime = 3000, verbose = false, errors = true, info = true } = {
    directory : './', responseDelay : 0, cacheLifetime : 3000, verbose : false, errors : true, info: true
}) {
    ...
    function configRoute(req, res, next){
        req.servicePath = path.join(__dirname, '../',directory, (/^([^?]+)/).exec(req.url)[0]);
